...
from sidm.tools.utilities import dR, lxy, rho, check_bits, returnBitMapTArrayPhoton, numClose, dR_outer
...
obj_cut_defs = {
    ...
    "ljs": {
        ...
        "only2muLj": lambda objs: objs["ljs"].muon_n == 2,
        "2dsaMuLj": lambda objs: objs["ljs"].dsaMu_n >= 2,
    ...
    "genAs_toMu": {
        ...
        "test close dsaMu": lambda objs:  numClose(objs["genAs_toMu"],objs["dsaMuons"],0.4) >= 1,
    ...
    "dsaMuons": {
        ...
        "DT + CSC hits > 12": lambda objs, dsa: (dsa.trkNumDTHits
                                            + dsa.trkNumCSCHits) > 12,
        "ifcsczero": lambda objs, dsa: ak.where(((dsa.trkNumCSCHits == 0)
